        ...
        match key.as_str() {
            "paths" => {
                ...
                for (path, json_file) in p {
                    ...
                    let json_path = Path::new(&config.data_dir.clone().unwrap())
                        .join(json_file)
                        .display()
                        .to_string();
                    let _ = fs::metadata(&json_path)
                        .expect(format!("{} is missing", json_path).as_str());

                    let full_path =
                        format!("/{}/{}/", path_prefix, path.to_string()).replace("//", "/");
